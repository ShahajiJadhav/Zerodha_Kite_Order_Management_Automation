...
"""
Local (verbose) HFT bot for Kite — COMPLETE version (modified per user request)

Changes made:
- Removed *initial market entry placement* when a Chartink signal is received. The bot no longer places market entry orders from signals.
- On each signal the bot now:
    * computes quantity from a fixed SIGNAL_AMOUNT (environment variable SIGNAL_AMOUNT, default 10000) and 5x leverage,
    * places a standing SL-M (using the robust place_slm_order path) using the previous completed 5m candle extreme as trigger,
    * sends a Telegram notification with symbol, side, suggested qty and LTP.
- Ensures uniqueness: the bot will only send/place one signal per (symbol, side) in a 30-minute window (SIGNAL_COOLDOWN_MINUTES; default 30).
- The scheduler/trailing logic remains unchanged and will continue to force-update SL-Ms for any real executed positions.

Note: Placing SL-M orders without an off-setting MIS position may behave differently depending on your broker. This script follows your request exactly (place SL-M on signal and trail thereafter), but BE CAUTIOUS: many brokers don't allow or will treat SL-M without existing position specially. Test in SIMULATION_MODE first.
"""
...
# New: Telegram + signal amount config
TELEGRAM_BOT_TOKEN = os.getenv("TELEGRAM_BOT_TOKEN", "")
TELEGRAM_CHAT_ID   = os.getenv("TELEGRAM_CHAT_ID", "")
SIGNAL_AMOUNT      = float(os.getenv("SIGNAL_AMOUNT", "10000"))  # fixed rupee amount to convert to qty
SIGNAL_LEVERAGE    = float(os.getenv("SIGNAL_LEVERAGE", "5"))     # leverage used when computing qty for signal
...
signal_created_slm = {}  # {order_id (str): datetime_created}
_all_instruments = None
symbol_to_token = {}
symbol_to_tick = defaultdict(lambda: 0.05)
allowed_symbols = set()
# persistence for signal-created SLMs
SIGNAL_SLM_PERSIST_FILE = os.path.join(os.path.dirname(__file__), "signal_created_slm.json")

# signal dedupe memory: {(symbol,side): datetime_sent}
last_signal_sent = {}
...
def _cleanup_pending():
    ...
    # prune tracked signal SLMs when cleaning up
    _prune_signal_created_slm()


def _prune_signal_created_slm():
    """Remove entries older than SIGNAL_COOLDOWN_MINUTES to avoid unbounded growth."""
    now = now_ist()
    to_delete = []
    for oid, ts in list(signal_created_slm.items()):
        if (now - ts) >= timedelta(minutes=SIGNAL_COOLDOWN_MINUTES):
            to_delete.append(oid)
    for oid in to_delete:
        signal_created_slm.pop(oid, None)
    if DEBUG and to_delete:
        log(f"[signal-reg] pruned {len(to_delete)} old signal-created SLMs")
    if to_delete:
        _save_persisted_signal_slm()
...
def _load_persisted_signal_slm():
    global signal_created_slm
    try:
        if os.path.exists(SIGNAL_SLM_PERSIST_FILE):
            with open(SIGNAL_SLM_PERSIST_FILE, "r") as f:
                raw = json.load(f)
            # convert keys to str and values to datetime
            for oid, ts in raw.items():
                try:
                    signal_created_slm[str(oid)] = datetime.fromisoformat(ts)
                except Exception:
                    # ignore bad entries
                    continue
            if DEBUG:
                log(f"[persist] loaded {len(signal_created_slm)} signal-created SLMs")
    except Exception as e:
        log(f"[persist] load failed: {e}")

def _save_persisted_signal_slm():
    try:
        # convert datetimes to isoformat strings
        serial = {oid: ts.isoformat() for oid, ts in signal_created_slm.items()}
        with open(SIGNAL_SLM_PERSIST_FILE, "w") as f:
            json.dump(serial, f)
        if DEBUG:
            log(f"[persist] saved {len(serial)} signal-created SLMs")
    except Exception as e:
        log(f"[persist] save failed: {e}")
...
def reconcile_orphan_orders():
    ...
    Skips recently-created signal SLMs tracked in signal_created_slm for a cooldown window.
    """
    try:
        ...
        for o in call_kite(kite.orders) or []:
            ...
            if o.get("variety") != "regular":
                continue
            if o.get("status") not in ("TRIGGER PENDING", "OPEN",
                                       "OPEN PENDING", "PUT ORDER REQUEST RECEIVED",
                                       "VALIDATION PENDING", "MODIFY VALIDATION PENDING",
            ...
            # If this SL-M was created by our signal flow and is still within cooldown, skip cancelling
            created_ts = signal_created_slm.get(oid)
            if created_ts:
                age_min = (now_ist() - created_ts).total_seconds() / 60.0
                if age_min < SIGNAL_COOLDOWN_MINUTES:
                    if DEBUG:
                        log(f"[reconcile] skipping signal-created SLM {sym} oid={oid} age={age_min:.1f}m")
                    continue
                else:
                    # old — forget it, allow normal reconciliation and remove from our registry
                    signal_created_slm.pop(oid, None)
                    try:
                        _save_persisted_signal_slm()
                    except Exception:
            ...
            # If there's no corresponding position, cancel it
...
def fetch_chartink_signals(scan_type: str, payload: dict):
    ...
    try:
        r = requests.post(
            ...
            headers=headers,
            json=payload,        # ✅ send JSON, not form data
            cookies=cookies,     # ✅ attach cookies properly
            timeout=12,
        )
        r.raise_for_status()
        data = r.json()
...
def gather_signals():
    ...
    buy_payload  = {"scan_clause":'''( {1339018} ( ( {cash} ( ( {cash} ( latest close > 25 and latest close < 80 and( {cash} ( [0] 10 minute volume > 50000 or( {cash} ( [0] 5 minute volume > 30000 and [-1] 5 minute volume > 20000 ) ) ) ) ) ) or( {cash} ( latest close > 80 and latest close < 150 and( {cash} ( [0] 10 minute volume > 45000 or( {cash} ( [0] 5 minute volume > 25000 and [-1] 5 minute volume > 20000 ) ) ) ) ) ) or( {cash} ( latest close > 150 and latest close < 500 and( {cash} ( [0] 10 minute volume > 35000 and( {cash} ( [0] 5 minute volume > 14000 and [-1] 5 minute volume > 14000 ) ) ) ) ) ) or( {cash} ( latest close > 500 and( {cash} ( [0] 10 minute volume > 30000 or( {cash} ( [-1] 5 minute volume > 10000 and [0] 5 minute volume > 25000 ) ) ) ) ) ) ) ) and [0] 5 minute close > [0] 5 minute supertrend( 18 , 1.2 ) and [-1] 5 minute count( 20, 1 where [-1] 5 minute close > [-1] 5 minute supertrend( 18 , 1.1 ) ) > 12 and [0] 5 minute close > [-1] 5 minute max( 9 , [-1] 5 minute close ) * 0.9965 and [0] 1 hour volume >= 200000 and [0] 5 minute close > 1 day ago close * 1.004 and [-3] 5 minute count( 7, 1 where abs( [-3] 5 minute close - [-3] 5 minute open ) < [-3] 5 minute close * 0.004 ) > 4 and abs( [0] 5 minute close - [0] 5 minute open ) > [0] 5 minute open * 0.0025 and [0] 5 minute close > [0] 5 minute open and( {cash} ( ( {cash} ( latest volume < 1000000 and [0] 5 minute {custom_indicator_165412_start}\"sma(  stochrsi( 18 ) , 3 )\"{custom_indicator_165412_end} < 75 ) ) or latest volume > 1000000 ) ) ) )'''}  # trimmed in this file for readability
    sell_payload = {"scan_clause": '''( {1339018} ( ( {cash} ( ( {cash} ( latest close > 25 and latest close < 80 and( {cash} ( [0] 10 minute volume > 50000 or( {cash} ( [0] 5 minute volume > 30000 and [-1] 5 minute volume > 20000 ) ) ) ) ) ) or( {cash} ( latest close > 80 and latest close < 150 and( {cash} ( [0] 10 minute volume > 45000 or( {cash} ( [0] 5 minute volume > 25000 and [-1] 5 minute volume > 20000 ) ) ) ) ) ) or( {cash} ( latest close > 150 and latest close < 500 and( {cash} ( [0] 10 minute volume > 35000 and( {cash} ( [0] 5 minute volume > 14000 and [-1] 5 minute volume > 14000 ) ) ) ) ) ) or( {cash} ( latest close > 500 and( {cash} ( [0] 10 minute volume > 30000 or( {cash} ( [-1] 5 minute volume > 10000 and [0] 5 minute volume > 25000 ) ) ) ) ) ) ) ) and [-1] 5 minute count( 20, 1 where [-1] 5 minute close < [-1] 5 minute supertrend( 18 , 1.1 ) ) > 13 and [0] 5 minute volume > [0] 5 minute sma( [0] 5 minute volume , 9 ) and [0] 5 minute close < [-1] 5 minute min( 9 , [-1] 5 minute close ) * 1.003 and [0] 1 hour volume >= 200000 and [0] 5 minute close < 1 day ago close * 0.985 and [-3] 5 minute count( 10, 1 where abs( [-3] 5 minute close - [-3] 5 minute open ) < [-3] 5 minute close * 0.004 ) > 5 and abs( [0] 5 minute close - [0] 5 minute open ) > [0] 5 minute open * 0.0025 and [0] 5 minute close < [0] 5 minute open and( {cash} ( ( {cash} ( latest volume < 1000000 and [0] 5 minute {custom_indicator_165412_start}\"sma(  stochrsi( 18 ) , 3 )\"{custom_indicator_165412_end} < 75 ) ) or latest volume > 1000000 ) ) ) )'''}
...
# ==================== Orders ==================== #
def detect_manual_entries_and_place_sl(prev_snapshot: dict, curr_snapshot: dict):
    """
    Detect newly created MIS positions (manual execution) by comparing prev -> curr snapshot.
    For each new position:
      - ensure an SL-M exists (using previous completed 5m candle extreme or fallback)
      - if missing, place SL-M immediately and log
    This does NOT place any market entries — it only reacts to executed positions.
    """
    try:
        prev_syms = set(prev_snapshot.keys())
        curr_syms = set(curr_snapshot.keys())

        # New positions that appeared since last snapshot
        new_entries = curr_syms - prev_syms
        if not new_entries:
            return

        for sym in new_entries:
            pos = curr_snapshot.get(sym)
            if not pos:
                continue
            # Skip if not MIS (fetch_active_positions already only includes MIS)
            direction = pos.get("direction")
            qty = int(pos.get("qty", 0) or 0)
            if qty <= 0:
                continue

            # 1) If an SL-M already exists matching the required transaction type, leave it
            expected_trans = side_to_exit_trans(direction)
            existing_slm = fetch_symbol_slm_from_broker(sym, expected_trans)
            if existing_slm:
                log(f"[manual-entry] {sym} {direction} detected — existing SL-M oid={existing_slm.get('order_id')} found; skipping create.")
                continue

            # 2) Compute trigger from previous completed 5m candle (or fallback)
            trig, last = compute_sl_and_meta(sym, direction)
            if not trig:
                trig = _fallback_sl(sym, direction)

            if not trig:
                log(f"[manual-entry] {sym} {direction} detected but cannot compute trigger — skipping SLM create.")
                continue

            # 3) Place SL-M sized to the executed qty
            # Use pos qty to ensure SL matches actual position size
            pseudo_pos = {"qty": qty, "direction": direction}
            oid = None
            if exits_allowed_now():
                oid = place_slm_order(sym, direction, qty, trig)
            else:
                log(f"[manual-entry] {sym} {direction} detected after cutoff — not creating SL-M")

            if oid:
                # Optionally track created SL-Ms so reconcile won't cancel signal-created SLs (reuse registry)
                try:
                    # store minimal metadata so reconcilers/trailers can find them after restarts
                    signal_created_slm[str(oid)] = {"symbol": sym, "side": direction, "created": now_ist()}
                    _save_persisted_signal_slm()
                except Exception:
                    pass
                log(f"[manual-entry] {sym} {direction} qty={qty} -> SLM placed oid={oid} trig={trig}")
            else:
                log(f"[manual-entry] {sym} {direction} qty={qty} -> SLM create FAILED (trig={trig})")

    except Exception as e:
        log(f"[detect_manual_entries_and_place_sl] error: {e}")
...
def update_or_replace_slm(symbol: str, pos: dict, new_trig: float) -> str:
    ...
    try:
        for o in call_kite(kite.orders) or []:
            ...
            if o.get("tradingsymbol", "").upper() != symbol.upper():
                continue
            if o.get("status") not in ("OPEN", "TRIGGER PENDING", "OPEN PENDING",
                                       "PUT ORDER REQUEST RECEIVED", "VALIDATION PENDING",
                                       "MODIFY VALIDATION PENDING", "MODIFY REQUEST RECEIVED"):
                continue
            if o.get("order_type") not in ("SL", "SL-M"):
                continue
            if o.get("transaction_type") != expected_trans:
...
# -------------------- NEW: signal handling (no market entry) -------------------- #

def _compute_qty_from_fixed_amount(ltp: float) -> int:
    """Compute qty using SIGNAL_AMOUNT and SIGNAL_LEVERAGE every time we fetch a signal."""
    try:
        if ltp <= 0:
            return 0
        qty = int((SIGNAL_AMOUNT * SIGNAL_LEVERAGE) // ltp)
        return max(0, qty)
    except Exception as e:
        log(f"[_compute_qty_from_fixed_amount] {e}")
        return 0
...
def send_telegram_message(text: str):
    if not TELEGRAM_BOT_TOKEN or not TELEGRAM_CHAT_ID:
        if DEBUG:
            log("[telegram] token/chat not configured — skipping message")
        return False
    try:
...
def should_send_signal_now(symbol: str, side: str) -> bool:
    key = (symbol.upper(), side.upper())
    t0 = last_signal_sent.get(key)
    if not t0:
        return True
    return (now_ist() - t0) >= timedelta(minutes=SIGNAL_COOLDOWN_MINUTES)


def record_signal_sent(symbol: str, side: str):
    key = (symbol.upper(), side.upper())
    last_signal_sent[key] = now_ist()


def handle_chartink_signal(sig: dict):
    """Process one Chartink signal.

    - calculate qty from fixed amount
    - place an SL-M order (no market entry)
    - send Telegram message with the computed qty and LTP
    - enforce uniqueness per (symbol,side) for SIGNAL_COOLDOWN_MINUTES
    """
    ...
    if not should_send_signal_now(sym, side):
        if DEBUG:
            log(f"[signal] skipped (dedupe) {sym} {side}")
        return

    if sym not in allowed_symbols:
        if DEBUG:
            log(f"[signal] {sym} not in allowed 5x universe — skipped")
        return

    ltp = get_ltp(sym)
    qty = _compute_qty_from_fixed_amount(ltp)
    if qty <= 0:
        log(f"[signal] {sym} computed qty <=0 (ltp={ltp}) — skipped")
        return

    # Place SL-M using the previous closed 5m candle extreme as trigger
    trig, last = compute_sl_and_meta(sym, side)
    if not trig:
        trig = _fallback_sl(sym, side)

    pseudo_pos = {"qty": qty, "direction": side}
    act = "SKIPPED-SLM"
    oid = None
    if trig and exits_allowed_now():
        oid = place_slm_order(sym, side, qty, trig)
        act = "SLM-PLACED" if oid else "SLM-FAILED"
        if oid:
            try:
                signal_created_slm[str(oid)] = now_ist()
                _save_persisted_signal_slm()   # persist immediately so restarts don't forget
            except Exception:
                pass
    else:
        if DEBUG:
            log(f"[signal] {sym} trig not available or after cutoff — not placing SLM")

    # Send Telegram notification (with qty recomputed each time)
    text = (f"Signal {side} {sym} — qty={qty} @ LTP~{ltp:.2f}\nSL-TRIG={trig}\nAction={act}")
    sent = send_telegram_message(text)
    if sent:
        record_signal_sent(sym, side)
        log(f"[signal] notified {sym} {side} qty={qty} trig={trig} oid={oid}")
    else:
        log(f"[signal] telegram send failed for {sym} {side}")
...
def trail_slm_positions():
    ...
    # >>> IMPORTANT: take a snapshot so mutation in other threads won't blow up iteration
    ...
    for symbol, pos in positions_snapshot.items():
        ...
        # 3) Update or replace SL-M to match new_trig
        if exits_allowed_now():
...
def place_market_entry(symbol, side):
    """Place a market entry, immediately place SL via robust path, then *immediately* check breach and exit if needed."""
    sym = symbol.upper()
    with entry_lock:
        if not before_3pm():
        ...
        # --- NEW: enforce 1% gap rule with prev candle extreme --- #
        trig, last = compute_sl_and_meta(sym, side)
        ...
        if side.upper() == "SELL" and ltp_before > trig:
            ...
            return None
        if gap_pct > 4.3:
            log(f"[entry] {sym} gap {gap_pct:.2f}% > 1% — skipped.")
            return None
    ...
    try:
        ...
        # 1) Immediate SL from previous closed candle (robust path)
        if not trig:
            trig = _fallback_sl(sym, side)
            last_ts = last["ts"] if last else "NA"
        else:
        ...
        if trig:
            pseudo_pos = {"qty": qty, "direction": side}
            act = update_or_replace_slm(sym, pseudo_pos, trig)
            log(f"[diag-entry] {sym} dir={side} prev_ts={last_ts} trig={trig} action={act}")
...
def do_trailing_cycle(prev_snapshot):
    ...
    # detect manual executions (new positions) and create SL-M if missing
    try:
        detect_manual_entries_and_place_sl(prev_snapshot if prev_snapshot else {}, curr)
    except Exception as e:
        log(f"[do_trailing_cycle] detect manual entries failed: {e}")
...
def main_loop():
    global _prev_snapshot
    log("[main] starting bot (local verbose) — modified: signals place SLM + notify, no market entry…")
    load_instruments_once()
    load_allowed_symbols_5x()
    _load_persisted_signal_slm()
    ...
    while True:
        ...
        # React to manual fills quickly in main loop as well
        try:
            detect_manual_entries_and_place_sl(_prev_snapshot if _prev_snapshot else {}, curr)
        except Exception as e:
            log(f"[main] detect manual entries failed: {e}")
        ...
        if before_3pm():
            sigs = gather_signals()
            for sig in sigs:
                # previously we called place_market_entry(); now we only handle signal by
                # computing qty, placing SLM and notifying Telegram (deduped per side for 30m)
                handle_chartink_signal(sig)
