...
Purpose:
 ...
 - Additionally: run a fast order-watcher every 15 seconds to detect missing SLs for active positions
...
Notes:
 - This is a focused helper: it does not place entries. It assumes MIS intraday positions already exist.
 - It intentionally mirrors robust behaviors from your HFT bot: safe retries, token caching, and conservative checks.
...
# ORDER_POLL_INTERVAL kept for backward compatibility but fast watcher will randomize between 7-12s
ORDER_POLL_INTERVAL = 15  # seconds for fast order watcher
...
# ---------------- Fast order watcher (every ~7-12s) -----------------

def fast_order_watcher(stop_event: threading.Event):
    """Runs in background. Every ~7-12 seconds checks active MIS positions and ensures
    there is at least one SL-M for each (matching transaction side). If missing, it computes SL and places it.
    Additionally, it cancels SLM/SL orders for symbols that have no active MIS position (orphan cancel),
    providing low-latency orphan cleanup between the 5-min cadence reconcile.
    """
    print('[watcher] fast order watcher started')
    while not stop_event.is_set() and before_cutoff():
        try:
            positions = fetch_active_positions()
            active_symbols = set(positions.keys())

            # Ensure each active position has an SL-M
            for symbol, pos in positions.items():
                expected_trans = 'SELL' if pos['direction'].upper() == 'BUY' else 'BUY'
                slm = fetch_symbol_slm_from_broker(symbol, expected_trans)
                if slm:
                    # SL exists — nothing to do
                    continue
                # missing SL: compute using prev 5m candle (fallback to LTP-based if unavailable)
                new_trig, last = compute_sl_and_meta(symbol, pos['direction'])
                if not new_trig:
                    # fallback: small offset from LTP
                    try:
                        q = call_kite(kite.quote, f"NSE:{symbol}")
                        ltp = float(q[f"NSE:{symbol}"]['last_price'])
                        raw = ltp * (1 - 0.00003) if pos['direction'].upper() == 'BUY' else ltp * (1 + 0.00003)
                        new_trig = round_to_tick(raw)
                        print(f"[watcher] fallback SL for {symbol} -> {new_trig}")
                    except Exception as e:
                        print(f"[watcher] cannot compute fallback SL for {symbol}: {e}")
                        continue
                # place SL
                oid = place_slm_order(symbol, pos['direction'], pos['qty'], new_trig)
                print(f"[watcher] placed SL for {symbol}: oid={oid} trig={new_trig}")

            # Fast orphan cancellation: cancel SL/SL-M for symbols that no longer have active MIS positions
            try:
                ...
                print(f"[watcher-reconcile] listing orders failed: {e}")

        except Exception as e:
            print(f"[watcher] error: {e}")

        # randomize next poll between 7 and 12 seconds
        wait_s = random.randint(7, 12)
